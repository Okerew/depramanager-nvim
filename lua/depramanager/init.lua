...
local function get_rust_outdated_async(callback)
    ...
    local cmd = "cargo update --dry-run"
    ...
    run_command_async(cmd, function(code, stdout_data, stderr_data)
        ...
        for line in output:gmatch("[^\n]+") do
            local name, from_version, to_version = line:match("Updating ([^%s]+) v([^%s]+) %-> v([^%s]+)")
            if name and from_version and to_version then
                packages[name] = to_version
                table.insert(display_results,
                    string.format("🦀 %s: %s → %s", name, from_version, to_version))
            end
        end
